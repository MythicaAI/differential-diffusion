...
class StableDiffusionDiffImg2ImgPipeline(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        threshold = 0.3          # 50 % white

        # map is a float tensor in [0,1]
        map = torch.where(
            map >= threshold,          # condition
            torch.ones_like(map),      # value where condition is True  → 1.0
            map                        # value where condition is False → keep original
        )
        ...
        if self.unet.config.time_cond_proj_dim is not None:
            ...
            ).to(device=device, dtype=original_with_noise.dtype)
